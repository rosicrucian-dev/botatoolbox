 ...
 *
 * Caching model:
 *   - Precache (install): every HTML page + /_next/static + icons + the
 *     recordings SEARCH INDEX — ~14MB, so the whole study tool works in
 *     airplane mode after one visit, transcript search included. Entries
 *     carry a content hash; on updates only changed files are
 ...
 *   - Versioned (as browsed): the search snippet sidecars, whose URLs
 *     carry this build's content stamp — cache-first with no
 *     revalidation, since a given URL can never change. Superseded
...
// Content stamp of this build's snippet sidecars, injected by gen-sw.ts
// from the generated index. Sidecar URLs carry it as `?v=`; anything
// cached under a different stamp is last deploy's text and is dropped
// on activate.
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    (async () => {
      ...
      // Drop snippet sidecars from earlier deploys. They are served
      // cache-first and never revalidated, so the stamp in their URL is
      // the only thing that retires them; without this sweep the old
      // copies would accumulate for as long as storage lasts.
      const runtime = await caches.open(RUNTIME)
      for (const request of await runtime.keys()) {
        const url = new URL(request.url)
        if (
          url.pathname.startsWith('/data/recordings-snippets/') &&
...
// The search index sits at a stable URL, so it can't be cache-first in
// general — a deploy would never reach the reader. It is precached
// instead (rev-tracked, refreshed only when its hash changes), and a
// precache hit is authoritative: serving it costs no network at all,
// where stale-while-revalidate re-downloaded ~1.7MB in the background
// every session that opened search.
...
// Snippet sidecars are immutable for the life of a deploy — their URL
// carries the build's content stamp — so a hit is final and never
// revalidated. Stale stamps are swept on activate.
...
self.addEventListener('fetch', (event) => {
  ...
  } else if (url.pathname === '/data/recordings-search.json') {
    event.respondWith(handleSearchIndex(request, event))
  } else if (url.pathname.startsWith('/data/recordings-snippets/')) {
