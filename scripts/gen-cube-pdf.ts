...
//
//   npm run gen:cube-pdf            # writes local/files/cube-of-space-net-{a4,letter}.pdf
...
import { DEFAULT_MAJOR_STYLE } from '../src/content/data/tarot-styles.ts'
import { getColor } from '../src/lib/colors.ts'
...
// ---------------------------------------------------------------------------
// DATA — attributions from content/data/cube-of-space.json (shared with
// CubeCanvas.tsx so the paper cube matches the 3D one); 2D layout local.
// ---------------------------------------------------------------------------
interface Card {
  num: number
  slug: string
...
// The print deliberately uses the traditional FLO palette (not the app's
// current default) — shared table in src/lib/colors.ts, no local copy.
const colorOf = (slug: string) =>
  getColor(cardBySlug[slug].color, 'flo') ?? '#888'

interface CubeData {
  edges: Array<{ id: string; cardSlug: string; flow: string }>
  faces: Array<{
    id: string
    cardSlug: string
...
const cubeData: CubeData = JSON.parse(
  fs.readFileSync(path.join(ROOT, 'content/data/cube-of-space.json'), 'utf8'),
)

// 12 edges -> zodiac card
const edges: Record<string, string> = Object.fromEntries(
  cubeData.edges.map((e) => [e.id, e.cardSlug]),
)
...
// 2D-layout geometry per face; attribution + borders come from the JSON.
const faceLayout: Record<string, { set: FaceDef['set']; cardRotZ?: number }> = {
  east: { set: 'lateral' },
  west: { set: 'lateral' },
  above: { set: 'above', cardRotZ: -Math.PI / 2 },
  below: { set: 'below', cardRotZ: -Math.PI / 2 },
  north: { set: 'lateral' },
  south: { set: 'lateral' },
}
const faces: Record<string, FaceDef> = Object.fromEntries(
  cubeData.faces.map((f) => [f.id, { ...f, ...faceLayout[f.id] }]),
)
...
const baseVec: Record<Side, [number, number]> = {
  top: [0, -1],
  right: [1, 0],
  bottom: [0, 1],
  left: [-1, 0],
...
const localSideToNet = (side: Side, rot: number) =>
  dirName(rotCW(baseVec[side], rot))
const cellKey = (c: number, r: number) => `${c},${r}`
// the other face that shares this edge
const faceSharingEdge = (edgeId: string, exceptId: string) =>
  Object.values(faces).find(
    (f) => f.id !== exceptId && SIDES.some((s) => f.borders[s] === edgeId),
  )!
...
const TAB_OVERRIDE: Record<string, string> = {
  NE: 'east',
  SE: 'east', // Empress (top square)
  NW: 'west',
  SW: 'west', // Wheel (middle square)
  'T-N': 'above',
  'T-S': 'above',
  'T-E': 'above', // Magician (bottom square)
}
...
for (const s of perimeter)
  (byEdge.get(s.edgeId) ?? byEdge.set(s.edgeId, []).get(s.edgeId)!).push(s)
...
for (const [edgeId, pair] of byEdge) {
  ...
  const tab = forced
    ...
    : pair.sort(
        (a, b) =>
          (tabsPerCell.get(a.p.faceId) ?? 0) -
          (tabsPerCell.get(b.p.faceId) ?? 0),
      )[0]
  tab.kind = 'tab'
...
function renderFaceBase(p: Placement): string {
  ...
  for (const side of ['top', 'bottom', 'left', 'right'] as Side[]) {
    const pts = bandPts[side]
      .map(([x, y]) => `${fx(x).toFixed(2)},${fy(y).toFixed(2)}`)
      .join(' ')
    parts.push(
      `<polygon points="${pts}" fill="${colorOf(edges[f.borders[side]])}"/>`,
    )
  }
...
function renderFaceCards(p: Placement): string {
  ...
  for (const side of ['top', 'bottom', 'left', 'right'] as Side[]) {
    ...
    const dx = (0.5 - ucx) * Wf
    const dy = -(0.5 - ucy) * Hf
    const cx = fx(spec.position[0]),
      cy = fy(spec.position[1])
...
const FOLD_LINE =
  'stroke="#ffffff" stroke-opacity="0.5" stroke-width="0.5" stroke-linecap="round" fill="none"'
...
const CUT_LINE =
  'stroke="#111" stroke-opacity="0.5" stroke-width="0.5" fill="none"'
...
function renderTopology(): TopoParts {
  ...
  const D = CONFIG.tabDepthMM,
    INS = CONFIG.tabInsetMM,
    BL = CONFIG.bleedMM
  ...
  for (const s of sideInfos) {
    ...
    if (s.kind === 'fold') {
      foldLines.push(
        `<line x1="${a[0]}" y1="${a[1]}" x2="${b[0]}" y2="${b[1]}" ${FOLD_LINE}/>`,
      )
    } else if (s.kind === 'land') {
      ...
      bleeds.push(
        `<polygon points="${a[0]},${a[1]} ${a[0] + n[0] * BL},${a[1] + n[1] * BL} ${b[0] + n[0] * BL},${b[1] + n[1] * BL} ${b[0]},${b[1]}" fill="${colorOf(edges[s.edgeId])}"/>`,
      )
      cutLines.push(
        `<line x1="${a[0]}" y1="${a[1]}" x2="${b[0]}" y2="${b[1]}" ${CUT_LINE}/>`,
      )
    } else {
      ...
      tabFills.push(
        `<polygon points="${a[0]},${a[1]} ${o1[0]},${o1[1]} ${o2[0]},${o2[1]} ${b[0]},${b[1]}" fill="${colorOf(edges[s.edgeId])}" fill-opacity="0.9"/>`,
      )
      foldLines.push(
        `<line x1="${a[0]}" y1="${a[1]}" x2="${b[0]}" y2="${b[1]}" ${FOLD_LINE}/>`,
      ) // base
      cutLines.push(
        `<polyline points="${a[0]},${a[1]} ${o1[0]},${o1[1]} ${o2[0]},${o2[1]} ${b[0]},${b[1]}" ${CUT_LINE}/>`,
      )
    }
  }
  return {
...
function renderChrome(): string {
  ...
  const lines: string[] = []
  lines.push(
    `<text x="${tl.x}" y="${tl.y + 6}" ${serif} font-size="4" letter-spacing="0.5" fill="#111">CUBE OF SPACE</text>`,
  )
  lines.push(
    `<line x1="${tl.x}" y1="${tl.y + 10}" x2="${tl.x + 40}" y2="${tl.y + 10}" stroke="#111" stroke-width="0.3"/>`,
  )
  instr.forEach((t, i) =>
    lines.push(
      `<text x="${tl.x}" y="${tl.y + 16 + i * 4.3}" ${sans} font-size="3.2" fill="#222">${t}</text>`,
    ),
  )
  return lines.join('')
}
...
function buildSvg(): { svg: string; w: number; h: number } {
  ...
  const net = MIR
    ? `<g transform="translate(${w} 0) scale(-1 1)">${netInner}</g>`
    : netInner
  const svg =
...
console.log(
  `wrote ${path.join(scratch, 'cube-net.png')} (${w.toFixed(0)}x${h.toFixed(0)}mm @ ${dpi}dpi)`,
)
...
if (!preview) {
  ...
  for (const [name, [pw, ph]] of Object.entries(PAGES)) {
    ...
    const out = path.join(ROOT, 'local/files', `cube-of-space-net-${name}.pdf`)
