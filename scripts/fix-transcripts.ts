...
const normalize = buildNormalize(
  JSON.parse(readFileSync(GLOSSARY, 'utf8')).normalize as Record<
    string,
    string
  >,
)
  ...
  const out = paras(original).map((p) => {
    ...
    if (norm !== p) {
      ...
      for (let i = 0; i < Math.min(norm.length, p.length); i++) {
        if (norm[i] !== p[i]) {
          glossaryHits.push(
            `${file.replace(/\.md$/, '')}: …${p.slice(Math.max(0, i - 20), i + 12)}… → …${norm.slice(Math.max(0, i - 20), i + 10)}…`,
          )
          break
        }
      }
...
if (glossaryHits.length) {
  ...
  if (glossaryHits.length > 15)
    console.log(`  … +${glossaryHits.length - 15} more`)
}
