...
for (const slug of slugs) {
  ...
  const published = splitParagraphs(
    readFileSync(join(BODY_DIR, `${slug}.md`), 'utf8'),
  )
  ...
  if (matches) {
    timings[slug] = derived.map((d) =>
      Math.max(0, Math.round(d.start * 100) / 100),
    )
  } else {
...
if (mismatched.length) {
  ...
  if (mismatched.length > 20)
    console.log(`      … +${mismatched.length - 20} more`)
}
if (noSource.length) {
