...
// and it's validated at build time rather than parsed at runtime. It's large,
// so it's served as a static file in /public and FETCHED on demand rather than
// bundled (see src/content/data/gematria-words.ts).
// One Strong's entry for a given consonantal spelling. Because homographs
// share consonants (and therefore gematria value), a single spelling can map
// to several of these — all legitimate inhabitants of the same number. The
// pointed `lemma` shows which vocalization carries which meaning.
export const GematriaStrongsSchema = z.object({
  // Strong's number, e.g. "H259" — for traceability/verification.
  num: z.string(),
  // The vocalized (pointed) lemma, e.g. "אֶחָד".
  lemma: z.string(),
  // Strong's gloss for that lemma.
  def: z.string(),
})
...
export const GematriaWordSchema = z.object({
  ...
  crowley: z.string(),
  // Every Strong's word sharing this spelling (deduped by definition). A
  // gematria tool surfaces possibilities, so we list them all rather than
  // pick one — see scripts/build-gematria-words.ts.
  strongs: z.array(GematriaStrongsSchema).optional(),
})

// A Strong's-only word at a value — present in the Hebrew Bible but NOT in
// Crowley's Sepher Sephiroth. Same shape as a Crowley word minus the curated
// fields (no `crowley`, no `roman`); listed beneath the curated set so a
// gematria number shows every avenue, with Crowley's selection still first.
export const GematriaOtherSchema = z.object({
  hebrew: z.string(),
  strongs: z.array(GematriaStrongsSchema),
})
...
export const GematriaNumberEntrySchema = z.object({
  ...
  // Other Hebrew Bible words at this value, beyond Crowley's curated list.
  other: z.array(GematriaOtherSchema).optional(),
})
