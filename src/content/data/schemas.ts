...
// ---------- recordings.json ----------
//
// GENERATED by scripts/gen-recordings.ts from the local mlx-whisper pipeline
// output — do not hand-edit. One row per Ann Davies lecture; the transcript
// body lives in content/recordings/en/<slug>.md and renders at
// /recordings/<groupingSlug>/<slug>. English-only (no overlay); a single
// top-level data file like cube-of-space.json.
export const RecordingSchema = z.object({
  slug: z.string().describe('Globally-unique URL/file slug for this recording.'),
  title: z.string(),
  catalogNumber: z
    .string()
    .describe("BOTA tape/catalog number (e.g. '5C'), or '' if uncatalogued."),
  grouping: z.string().describe('Display name of the grouping (Services, a series…).'),
  groupingSlug: z.string().describe('URL segment for the grouping.'),
  durationSeconds: z.number().int().nonnegative(),
  audioPath: z
    .string()
    .describe(
      'Relative MP3 path (e.g. "services/5c-….mp3"); the base host is resolved ' +
        'at runtime via NEXT_PUBLIC_RECORDINGS_AUDIO_BASE (R2 in prod, local in dev).',
    ),
  hidden: z
    .boolean()
    .optional()
    .describe('Hidden from listings/sitemap; still reachable by direct URL.'),
})

// ---------- public/data/<collection>-search.json ----------
//
// GENERATED by scripts/gen-*-search.ts (via src/lib/search-engine's
// buildInvertedIndex) — a positional full-text index (word → [docIdx, count,
// …positions]) shared by every collection search. Fetched at runtime by the
// collection search dialog;
// validated at build inside each generator.
export const CollectionSearchIndexSchema = z.object({
  docs: z.array(
    z.object({
      id: z.string(),
      title: z.string(),
  ...
  words: z.record(z.string(), z.array(z.number())),
  lengths: z.array(z.number()),
  // Content stamp of this build's snippet sidecars — rides on their URLs as
  // `?v=` so a deploy invalidates the copies a reader's service worker holds.
  version: z.string(),
})
