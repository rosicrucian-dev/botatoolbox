...
export const DEFAULT_MAJOR_STYLE =
  MAJOR_STYLES.find((s) => s.id === PREFERRED_MAJOR_STYLE)?.id ??
  MAJOR_STYLES[0].id
...
export function majorAspectRatio(id: string): number {
  return (
    MAJOR_STYLES.find((s) => s.id === id)?.aspectRatio ??
    MAJOR_STYLES[0].aspectRatio
  )
}
export function minorAspectRatio(id: string): number {
  return (
    MINOR_STYLES.find((s) => s.id === id)?.aspectRatio ??
    MINOR_STYLES[0].aspectRatio
  )
}
