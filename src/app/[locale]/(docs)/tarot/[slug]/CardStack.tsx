...
export function CardStack({
  ...
  return (
    ...
    //
    // Row template `auto 1fr`: the header row stays at its natural height
    // and the meta row is flexible. This keeps the meta top-aligned beside
    // the tall image — a spanning item's excess height is absorbed by the
    // flexible row rather than split equally into both `auto` rows (which
    // would inflate the header row and push the meta down to the middle).
    <div className="grid grid-cols-1 gap-x-8 gap-y-4 md:grid-cols-[minmax(0,1fr)_minmax(0,min(40%,24rem))] md:grid-rows-[auto_1fr] md:items-start">
