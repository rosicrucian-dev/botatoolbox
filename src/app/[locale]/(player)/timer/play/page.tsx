...
export default function TimerPlayPage() {
  ...
  const slides = useMemo(() => {
    ...
    return [
      ...
      { label: t('player.timer.done'), kind: 'done' as const },
    ]
  }, [steps, t])
    ...
    <SlidePlayer
      ...
      renderFull={(slide, i) => {
        if (slide.kind === 'done') {
          return (
            <CheckCircleIcon
              aria-label={t('player.timer.done')}
          ...
          content = (
            <div className="text-center">
              ...
              <div className="mt-2 text-lg opacity-60 md:text-xl">
                {minutesLeft === 1
                  ? t('player.timer.minuteLeft')
                  : t('player.timer.minutesLeft')}
              </div>
            </div>
          )
        } else {
        ...
        return (
          <div className="flex flex-col items-center gap-10">
            ...
            <button
              ...
              aria-label={
                paused ? t('player.timer.resume') : t('player.timer.pause')
              }
              aria-pressed={paused}
