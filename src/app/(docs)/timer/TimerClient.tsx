  ...
  return (
    <div className={cardClass}>
      <div className="flex items-start justify-between gap-4">
        ...
        <StepControls index={index} count={count} onMove={move} onRemove={remove} />
      </div>
...
function BreathStepCard({
  ...
  return (
    <div className={cardClass}>
      ...
      <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-5">
        <NumberField label="Inhale" value={step.inhale} min={0} onChange={(inhale) => update({ inhale })} />
        <NumberField label="Hold" value={step.holdIn} min={0} onChange={(holdIn) => update({ holdIn })} />
        <NumberField label="Exhale" value={step.exhale} min={0} onChange={(exhale) => update({ exhale })} />
        <NumberField label="Hold" value={step.holdOut} min={0} onChange={(holdOut) => update({ holdOut })} />
        <NumberField label="Cycles" value={step.cycles} min={1} onChange={(cycles) => update({ cycles })} />
      </div>
      {invalid && (
...
export function TimerClient() {
  ...
  return (
    <article className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <PageHeading truncate>Timer</PageHeading>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between rounded-2xl px-5 py-4 ring-1 ring-zinc-900/10 ring-inset dark:ring-white/10">
            ...
            <button
              ...
              onClick={clearSteps}
              className="flex size-8 items-center justify-center rounded-lg text-zinc-500 ring-1 ring-zinc-900/10 ring-inset transition hover:text-red-600 dark:text-zinc-400 dark:ring-white/10 dark:hover:text-red-400"
            >
              <TrashIcon className="size-4" />
            </button>
          </div>
          ...
          {TIMER_PRESETS.map((preset) => (
            ...
            >
              ...
              <button
                ...
                className="absolute inset-0 rounded-2xl focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-inset focus-visible:outline-none"
              />
