...
export default async function GradePage({
  ...
  const prevHref = prev ? `/grades/${prev.slug}` : undefined
  const nextHref = next ? `/grades/${next.slug}` : undefined
  ...
  return (
    <article className="space-y-6">
      ...
      <PrevNextNav
        ...
        prev={prev ? { href: `/grades/${prev.slug}`, label: prev.name } : undefined}
        next={next ? { href: `/grades/${next.slug}`, label: next.name } : undefined}
      />
    </article>
  )
}
