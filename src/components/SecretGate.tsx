...
// layout in this means: locked visitors get bounced to /members-only;
// unlocked visitors see the children normally. This is casual obscurity
// — the underlying HTML is still in the static export — but it's enough
// friction for accidental visitors.
export function SecretGate({ children }: { children: ReactNode }) {
  const { unlocked, hydrated } = useSecretMode()
  const router = useRouter()

  useEffect(() => {
    if (hydrated && !unlocked) router.replace('/members-only')
