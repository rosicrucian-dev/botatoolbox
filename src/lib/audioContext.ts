...
declare global {
  ...
  // Audio Session API (Safari 17+, not yet in lib.dom).
  interface Navigator {
...
export function ensureAudioContext(): AudioContext | null {
  ...
  // iOS 17+ Audio Session API: 'playback' routes our audio like a music
  // app, so tones play even when the Silent Mode switch is on — the #1
  // "the sound button is broken" report. Trade-off: playing a tone will
  // pause background music (Music/podcast apps), which is the standard
  // behavior users expect from meditation apps. SoundButton skips its
  // Silent Mode hint when this API is available.
  try {
