...
//
// Deliberately duplicates ASTROLOGY_PLANET_ORDER in content/data/astrology.ts
// rather than importing it: this module stays free of app-data imports so it
// can lift out into the standalone chart site. Keep the two in sync.
...
/**
 * Lunation quarter (1–4) for a chart, from the Moon's elongation east of
 * the Sun in ecliptic longitude — the same angle that defines the phases:
 * 0° is New, 90° First Quarter, 180° Full, 270° Last Quarter. Q1 spans
 * New → First Quarter (waxing crescent), Q2 → Full (waxing gibbous),
 * Q3 → Last Quarter (waning gibbous), Q4 → New (waning crescent).
 * Null if the chart lacks either luminary.
 */
export function lunarQuarter(chart: Chart): 1 | 2 | 3 | 4 | null {
  const sun = chart.bodies.find((b) => b.slug === 'sun')
  const moon = chart.bodies.find((b) => b.slug === 'moon')
  if (!sun || !moon) return null
  const elongation = (((moon.longitude - sun.longitude) % 360) + 360) % 360
  return (Math.floor(elongation / 90) + 1) as 1 | 2 | 3 | 4
}
