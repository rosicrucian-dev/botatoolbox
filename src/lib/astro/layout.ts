 ...
 */
// Round to 3 decimals when placing points. Math.sin/Math.cos can differ
// in their final floating-point digit across JS engines/platforms, so the
// server render and the browser would otherwise emit slightly different
// SVG `d` strings and React reports a hydration mismatch. 3 decimals is
// far below one pixel of the 1000-unit viewBox, so it's visually a no-op
// (and it trims the markup a little).
...
export function angleToPoint(longitude: number, radius: number): Point {
  ...
  return {
    x: roundCoord(CENTER.x + radius * Math.cos(rad)),
    y: roundCoord(CENTER.y - radius * Math.sin(rad)),
  }
}
...
// Format a degree value as `7°46′`. Degrees AND minutes truncate (never
// round up), per astrological convention — rounding minutes can produce
// impossible readings like `30°00′` within a sign (a position at 29.996°
// belongs to this sign; 60-carrying it misplaces it in the next).
export function formatDegreesMinutes(value: number): string {
