...
//
// DELIBERATELY NOT the shared search-engine, though it would unify this with
// the collection dialog. That engine's tokenizer matches `[\p{L}][\p{L}']*` —
// letters only — so every digit in a title is discarded, and 36 of the ~88
// detail titles carry one. "2 of Cups" through "10 of Cups" all tokenize to
// the single token `cups`: nine cards, one token stream, no way to rank the
// one you asked for. Substring matching is not a shortcut here, it is the
// correct matcher for short digit-bearing names, and it stays until the
// tokenizer keeps digits (which would change ranking in both full-text
// corpora and is a much larger decision than this search box).
